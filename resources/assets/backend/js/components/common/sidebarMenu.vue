<template>
  <Sider ref="side" hide-trigger collapsible :collapsed-width="60" v-model="isCollapsed" style="overflow:auto;">
    ...
    <template v-if="!showCo">
      <Menu active-name="1-2" theme="dark" width="auto" v-show="!showCo">
        <template v-for="item in $router.options.routes" v-if="!item.hidden">
          ...
          <Submenu :key="item.id" :name="item.name" v-else>
            <template slot="title">
              <Icon :type="item.icon"></Icon>
              <span>{{item.name}}</span>
            </template>
            <MenuItem :name="child.name" v-for="child in item.children"
              @click.native="$router.push(item.path + '/' + child.path)" :key="child.name"
    ...
    <template v-for="(item, index) in $router.options.routes" v-if="showCo">
      <div style="text-align: center;" :key="index" v-if="!item.hidden">
        <Dropdown transfer placement="right-start" :key="index" v-if="item.noDropdown">
          <Button class="co-menu" type="text">
        ...
        <Dropdown transfer placement="right-start" :key="index" v-else>
          ...
          <DropdownMenu style="width: 200px;" slot="list">
            <template v-for="(child, i) in item.children">
              <DropdownItem :name="child.name" :key="i" @click.native="$router.push(item.path + '/' + child.path)"
                v-if="!child.meta || !child.meta.hidden">
